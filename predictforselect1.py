#Copy the image with the highest confidence level to the specified result folder.
...
if __name__ == "__main__":
    ...
    for dir in dirlist:
        ...
        id = os.path.split(os.path.split(top_image[0])[0])[1]
        path = os.path.join(sa_path, id)
        
        if not os.path.exists(path):
            os.makedirs(path)

        shutil.copy(top_image[0], path + os.sep + os.path.split(top_image[0])[1])
